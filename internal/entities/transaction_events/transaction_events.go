...
func (te *TransactionEvents) ToPoint() interface{} {
	...
	return influxdb.NewPoint(
		...
		map[string]string{
			...
			"transaction_type":   safeString(te.TransactionType),
			"currency":           safeString(te.Currency),
			"payment_method":     safeString(te.PaymentMethod),
			"status":             safeString(te.Status),
		...
		map[string]interface{}{
			...
			"merchant_id":           string(te.MerchantID),
			"destination_account":   string(te.DestinationAccount),
			"ip_address":            string(te.IPAddress),
			"user_agent":            string(te.UserAgent),
			"app_version":           string(te.AppVersion),
			"endpoint":              string(te.Endpoint),
			"method":                string(te.Method),
			"geo_city":              string(te.GeoCity),
			"geo_coordinates":       string(te.GeoCoordinates),
			"geo_timezone":          string(te.GeoTimezone),
			"geo_postal":            string(te.GeoPostal),
			"geo_isp":               string(te.GeoISP),
			"os_version":            string(te.OSVersion),
			"browser_version":       string(te.BrowserVersion),
			"details":               detailsJSON,
		},
		te.Timestamp,
	)
