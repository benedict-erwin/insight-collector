...
import (
	...
	uaJob "github.com/benedict-erwin/insight-collector/internal/jobs/user_activities"
...
// SaveUserActivities handle saving UserActivities
func SaveUserActivities(c echo.Context) error {
	var req uaEntities.UserActivitiesRequest

	// set logger scope
	log := logger.WithScope("SaveUserActivities")
	...
	// Generate JobId
	jobID := generateUserActivitiesJobId(&req)

	// Job Payload
	payload := asynq.Payload{
		TaskId:   jobID,
		TaskType: uaJob.TypeUserActivitiesLogging,
		Data: uaEntities.UserActivitiesRequest{
			UserID:            req.UserID,
...
// generateUserActivitiesJobId for unique jobid
func generateUserActivitiesJobId(payload *uaEntities.UserActivitiesRequest) string {
	// Clean endpoint
	...
	return fmt.Sprintf("ua_%x", hash[:8])
}

// ListUserActivities handles paginated listing of user activities
func ListUserActivities(c echo.Context) error {
	var req v2oss.PaginationRequest

	// set logger scope
	log := logger.WithScope("ListUserActivities")
	...
	// Get query config for user activities
	queryConfig := uaEntities.GetQueryConfig()
	...
	var records []uaEntities.UserActivitiesResponse
	for _, record := range results {
		records = append(records, uaEntities.MapToUserActivitiesResponse(record))
	}
...
// DetailUserActivities get single record of UserActivities
func DetailUserActivities(c echo.Context) error {
	// Get encoded ID from path parameter
	encodedID := c.Param("id")

	// set logger scope
	log := logger.WithScope("DetailUserActivities")
	...
	if err != nil {
		log.Error().Err(err).
			...
			Msg("Failed to retrieve user activity")
	...
	structuredResponse := uaEntities.MapToUserActivitiesResponse(record)
